...
def APAAC(fastas, lambdaValue=30, w=0.05):
	...
	dataFile = re.sub('codes$', '', os.path.split(os.path.realpath(__file__))[0]) + r'\codes\data\PAAC.txt' if platform.system() == 'Windows' else re.sub('codes$', '', os.path.split(os.path.realpath(__file__))[0]) + '/codes/data/PAAC.txt'
