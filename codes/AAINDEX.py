...
def AAINDEX(fastas, **kw):
	...
	fileAAindex = re.sub('codes$', '', os.path.split(os.path.realpath(__file__))[0]) + r'\codes\data\AAindex.txt' if platform.system() == 'Windows' else re.sub('codes$', '', os.path.split(os.path.realpath(__file__))[0]) + '/codes/data/AAindex.txt'
	...
	encodings = []
	header = ['#']
	for pos in kw['POS']: #range(1, len(fastas[0][1]) + 1):
	...
	for i in fastas:
		...
		code = [name]
		subseq = [sequence[i-1] for i in kw['POS']]
		for aa in sequence:
			if aa == '-':
				for j in AAindex:
					code.append(0)
